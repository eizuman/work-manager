...
### Backup или restore остановился на чтении листа

Чтение backup больше не заменяет ошибки API пустыми данными. Сообщение с названием листа означает, что операция остановлена до регистрации новой копии или, для restore, до первой очистки целевых данных. Проверьте доступ к указанному spreadsheet, наличие листа и OAuth-сессию. Пустой существующий лист допустим и сам по себе не является ошибкой.

Успешное завершение preflight не делает restore транзакционным: сбой во время последующей записи целевых листов требует проверки фактического состояния в Google Sheets.
